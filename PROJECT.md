...
### external_food_matches
| 列名 | 类型 | 说明 |
|------|------|------|
| id | uuid (PK) | 自动生成 |
| user_id | uuid | 关联 auth.users |
| normalized_name | text | 归一化后的查询名称 |
| original_name | text | 原始查询词 |
| source | text | 当前固定为 `fatsecret` |
| fatsecret_food_id | text | FatSecret `food_id` |
| fatsecret_serving_id | text | FatSecret `serving_id` |
| fatsecret_food_name | text | FatSecret 返回的食物名称 |
| locale_region | text | 区域，默认 `US` |
| locale_language | text | 语言，默认 `en` |
| match_confidence | numeric(4,3) | 匹配置信度 |
...
| fatsecret_serving_id | text | FatSecret `serving_id` |
| fatsecret_food_name | text | FatSecret 返回的食物名称 |
| serving_description | text | 份量描述 |
| serving_grams | numeric(8,2) | 份量克重 |
| per100_kcal | integer | 换算后的每100g热量 |
| per100_protein | numeric(6,1) | 换算后的每100g蛋白质 |
| per100_carb | numeric(6,1) | 换算后的每100g碳水 |
| per100_fat | numeric(6,1) | 换算后的每100g脂肪 |
| locale_region | text | 区域，默认 `US` |
| locale_language | text | 语言，默认 `en` |
| cached_at | timestamptz | 缓存生成时间 |
